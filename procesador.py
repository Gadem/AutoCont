...
def extraer_datos_factura(ruta_archivo):
    ...
    try:
        ...
        # Preparar el diccionario de datos con los nuevos campos de impuestos
        datos = {
            'UUID': 'N/A', 
            'Fecha': comprobante.get('Fecha'),
            'SubTotal': comprobante.get('SubTotal'), # <-- Nuevo
            'Total': comprobante.get('Total'),
            'Moneda': comprobante.get('Moneda'),
            'TipoDeComprobante': comprobante.get('TipoDeComprobante'),
            
            # Campos de Impuestos que se calcularán/llenarán
        ...
        if impuestos_nodo is not None:
            # A. Extraer el Total General de Impuestos Trasladados
            ...
            # B. Iterar sobre los Traslados (para IVA, IEPS, etc.)
            ...
            if traslados_generales is not None:
                for traslado in traslados_generales.findall('cfdi:Traslado', NAMESPACES):
                    impuesto_tipo = traslado.get('Impuesto') # '002' para IVA
                    tasa_o_cuota = traslado.get('TasaOCuota')
                    importe_impuesto = traslado.get('Importe')

                    # Clasificación por tipo de impuesto y tasa (puedes ampliar esto)
                    if impuesto_tipo == '002' and tasa_o_cuota == '0.160000':
                        # Sumamos el IVA 16% (si hubiera varios en el XML)
...
def guardar_en_csv(datos_facturas, nombre_archivo='datos_facturas.csv'):
    ...
    # --- CAMBIOS CLAVE: NUEVOS ENCABEZADOS Y TODOS LOS CAMPOS INCLUIDOS ---
    fieldnames = [
        'UUID', 
        'Fecha', 
        ...
        'Total', 
        'Moneda', 
        'TotalImpuestosTrasladados', # Nuevo campo
        'IVA_Tasa16', # Nuevo campo
        'TipoDeComprobante',
        'Emisor_RFC', 
        'Emisor_Nombre', 
        'Receptor_RFC', 
